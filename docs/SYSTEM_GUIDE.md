...
### Prerequisites
*   **Python 3.8 or higher**: Download from [python.org](https://www.python.org/downloads/) (for Windows, check **"Add python.exe to PATH"** during installation).
*   **SQLite3**: Pre-bundled with Python's standard library.
...
2.  **Create a Virtual Environment**:
    *   **Windows**:
        ...
        python -m venv .venv
        ```
        *(Or `py -m venv .venv` if using the Windows Python launcher)*
    *   **Linux / macOS**:
        ```bash
        python3 -m venv .venv
        ```
...
3.  **Activate the Virtual Environment**:
    ...
    *   **Windows PowerShell**:
        ...
        ```
        *(Note: If script execution is restricted, run `Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope Process`)*
    *   **Git Bash on Windows**:
        ```bash
...
4.  **Install Required Dependencies**:
    The system requires Streamlit, Pandas, NumPy, and Plotly:
    ```bash
    pip install -r requirements.txt
    ```

5.  **Initialize and Seed Mock Data (Recommended)**:
    To populate the SQLite database (`storage_allocation.db`) with historical allocation records for testing the Dashboard and Performance Monitoring tabs, run:
    ```bash
    python mock_data.py
    ```
